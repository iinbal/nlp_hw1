...
def naive_softmax_loss_and_gradient(
    ...
    ### YOUR CODE HERE
    x = outside_vectors @ center_word_vec
    sf_x = softmax(x)
    y_o = sf_x[outside_word_idx]
    loss = -np.log(y_o)
...
def skipgram(current_center_word, outside_words, word2ind,
    ...
    ### YOUR CODE HERE
    c_ind = word2ind[current_center_word]
    c_vec = center_word_vectors[c_ind]
    for word in outside_words:
        ...
        loss+=curr_loss
        grad_center_vecs[c_ind]+= curr_g_center
